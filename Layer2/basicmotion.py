...
class BasicMotions:
    ...
    def turn_right(self):
        ...
        start = time.time()
        delay = 0.05
        while((ang_change < ((math.pi)/4) or not bool(self.s.read_ls()[1])) and not self.s.pickedup):
    ...
    def turn_around(self):
        ...
        start = time.time()
        delay = 0.05
        while((ang_change < ((math.pi*3)/4) or not bool(self.s.read_ls()[1])) and not self.s.pickedup):
    ...
    def sample(self):
        ...
        delay = 0.05
        self.motor.set_spin(-4.9)
        while((ang_change < math.pi*2-1 or not bool(1 in self.s.read_ls())) and not self.s.pickedup):
            #PID things
    ...
    def go_to_intersection(self):
        ...
        to_return = [[0,0,0],True]
        while(not at_intersection and not self.s.pickedup):
            sensors = self.s.read_ls()
            us_sensors = self.s.read_us()
            if us_sensors[1] < 20 and to_return[1] == True:
                to_return[1] = False
                self.motor.setvel(0,0)
                time.sleep(0.25)
                self.turn_around()
            ...
            if(sensors == [0,1,0]):
                ...
                to_return[0] = self.s.read_us()
                self.__intersection()
            else:
                self.motor.forward()
                time.sleep(0.25)
                distance_left = self.s.read_us()[0]
                distance_right = self.s.read_us()[2]
                if (distance_left < 30 and distance_right < 30):
                    self.wall_following(distance_left)
        return to_return

    def forward_until_blocked(self, distance):
        while((not self.s.read_us()[1] < distance and not self.s.read_us()[1] == 0) and not self.s.pickedup):
            self.motor.forward()
        self.motor.setvel(0,0)
    ...
    def wall_following(self, distance):
        ...
        while (not 1 in self.s.read_ls() and not self.s.pickedup):
            state = self.s.read_us()
            desired = distance
            d = state[0]
            e = d - desired
    ...
    def turn_right_no_line(self):
        ...
        start = time.time()
        delay = 0.05
        while(ang_change < ((math.pi)/2.5) and not self.s.pickedup):
            if(ang_change + 0.75 > ((math.pi)/4)):
                self.motor.set_spin(self.w/1.5)
            elif time.time()-start > 0.1:
                self.motor.set_spin(self.w)
